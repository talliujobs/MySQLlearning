...
## 主从复制
### MySQL复制的原理
MySQL服务器会生成一个二进制日志（需要配置），这个二进制日志中保存了所有对数据的写操作（增删改等）。
主库生成了的二进制日志文件，从库负责读取该文件，并在从库的本地执行执行二进制日志，那么所有写操作命令都会在从从库执行一遍，这样就是MySQL复制。
...
### 功能（优点）
1. 提高服务器的安全性、稳定性、容灾率（主服务器挂掉，从库依然可以使用）
2. 备份，为灾难恢复、数据还原提供更多选择，但需要注意：复制不能代替备份。
3. 读写分离，负载均衡。解决应用系统，数据库出现的性能瓶颈，采用数据库集群的方式来实现查询负载；通常情况下，一个系统中数据库的查询操作比更新操作要多得多，通过多台查询服务器将数据库的查询分担到不同的查询服务器上从而提高查询效率。
...
1 准备工作:
	...
	注意事项：
	* 主服务器的MySQL版本要等于或低于从服务器的MySQL版本。
	* 因为版本是向后兼容的，高版本兼容低版本。
	* 如果主库的MySQL版本高于从库，主库用到了新版本的特性的话，那么从库发生错误，导致复制失败。

2 同源策略
	1. 为了避免写入错误的操作，关闭主库的数据库服务器。
	2. 查看主服务器的“数据库、数据表、数据信息”的保存目录。
	3. 打包主服务器的数据目录，并复制到从库之中，并对从库数据目录覆盖
>|命令|描述|
>|:---|:---|
>|service mysql stop| 停止主数据库服务 |
>|tar -zcvf mysql.tar.gz ./mysql/  | 打包数据库目录 |
>|service mysql start | 启动数据库服务器 |
>|scp ./mysql.tar.gz root@112.124.46.122:/tmp | 将其推送到从库 |

	scp是一条命令，用于不同服务器间传送文件，语法如下：
	scp 文件 用户名@目标服务器:目录位置


3 主库中创建账号，授予权限
	1. 登录主库
	2. 创建用户并授予replication slave权限。
	```
	grant replication slave on 数据库.数据表 to '账号'@'IP地址' identified by '密码';
	```
4 配置主库
**配置前首先要备份**
```
...
7 从库启动复制
```
CHANGE MASTER TO MASTER_HOST = '主服务器地址',MASTER_USER = '用户', MASTER_PASSWORD='密码',MASTER_LOG_FILE = '主库二进制文件',MASTER_LOG_POS=二进制文件位置;
START SLAVE;
SHOW SLAVE STATUS;
